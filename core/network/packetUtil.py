...
class PacketSaver:
    ...
    def save_packets(self,file_index=1):
        ...
        while not self.event_stop.is_set():
            try:
                ...
                if len(packet_buffer) == self.packets_per_file:
                    ...
                    pcap_filename = f"capture_{self.iface}_{file_index}_{timestamp}.pcap"
    ...
    def stopCaptureThread(self):
        ...
        print("停止保存文件，所有线程已退出。")

    def startSaverThread(self):
        # 初始化文件序号
        file_index = 1
        self.saver_thread = threading.Thread(target=self.save_packets,args={file_index,})
        # 启动线程
